...
impl BasicMaterial {
    pub fn get_emission(&self) -> Colour {
        self.emission
    ...
    pub fn reflect_direction(&self, direction: Vec3, normal: Vec3) -> Option<Vec3> {
        ...
        let perfect_reflection =
            (direction - normal.scale(2.0 * normal.dot(&direction))).normalise();

        Some(
            self.reflective_sharpness
                .map_or(perfect_reflection, |sharpness| {
    ...
    pub fn refractive_ray_count(&self, efficacy: f64) -> f64 {
        self.refractive_sharpness
            .map_or(1.0, |sharpness| f64::ceil(efficacy / (1.0 + sharpness)))
    }
    pub fn refract_direction(&self, direction: Vec3, normal: Vec3) -> Option<Vec3> {
        ...
        let perfect_refraction = direction.scale(r) + normal.scale(r * c - f64::sqrt(determinant));

        Some(
            self.refractive_sharpness
                .map_or(perfect_refraction, |refractivity| {
                    BasicMaterial::wobble_vector(
                        perfect_refraction,
                        normal.scale(-1.0),
                        refractivity,
                    )
    ...
    pub fn refract_direction_out(&self, direction: Vec3, normal: Vec3) -> Option<(Vec3, bool)> {
        if !self.is_refractive() {
            return None;
        }
        assert!(direction.normalised() && normal.normalised());
        ...
        if determinant < 0.0 {
            ...
            let perfect_reflection = direction - normal.scale(2.0 * normal.dot(&direction));

            let vec_out = self
                .refractive_sharpness
                .map_or(perfect_reflection, |refractivity| {
                    Material::wobble_vector(perfect_reflection, normal.scale(-1.0), refractivity)
                });
            Some((vec_out, true))
        } else {
            let perfect_refraction =
                (direction.scale(r) + normal.scale(r * c - f64::sqrt(determinant))).normalise();

            let vec_out = self
                .refractive_sharpness
                .map_or(perfect_refraction, |refractivity| {
                    BasicMaterial::wobble_vector(perfect_refraction, normal, refractivity)
                });
            Some((vec_out, false))
        }
    }
    ...
    fn wobble_vector(vector: Vec3, normal: Vec3, factor: f64) -> Vec3 {
        ...
        let random_vec = Vec3::random_unit_vector();
        ...
        (vector + (random_hemisphere - vector).scale(ratio.powf(factor))).normalise()
    }
}
